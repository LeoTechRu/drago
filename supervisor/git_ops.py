...
def ensure_repo_present() -> None:
    ...
    subprocess.run(["git", "config", "user.name", "Drago"], cwd=str(REPO_DIR), check=True)
    subprocess.run(["git", "config", "user.email", "drago@users.noreply.github.com"],
                    cwd=str(REPO_DIR), check=True)
...
def import_test() -> Dict[str, Any]:
    r = subprocess.run(
        ["python3", "-c", "import drago, drago.agent; print('import_ok')"],
        cwd=str(REPO_DIR),
